...
let resposta1 = -1, resposta2 = -1, resposta3 = -1, resposta4 = -1, linguagens = []
...
while (resposta2 != 1 && resposta2 != 2) {
    if (resposta1 == 1) resposta2 = prompt("Qual dessas linguagens você quer aprender?\n1 - React   2 - Vue")
    else if (resposta2 == 2) resposta2 = prompt("Qual dessas linguagens você quer aprender?\n1 - C# 2 - Java")
    if (resposta2 != 1 && resposta2 != 2)
        alert("Digite '1' ou '2' para responder a pergunta!")
}

alert(`Incrível! Você está traçando um caminho de sucesso!\nAgora o próximo passo importante é decidir entre se especializar em ${(resposta1 == 1)? "Front-End": "Back-End"} ou se desenvolver mais na outra e se tornar um desenvolvedor Fullstack!`)
...
if (resposta3 == 1) linguagens.concat(prompt("Sobre seus futuros estudos que linguagem você gostaria de se especializar?"))
else linguagens.concat(prompt("Sobre seus futuros estudos que linguagem você gostaria de conhecer?"))

while (resposta4 != 1 && resposta4 != 2) {
    resposta4 = prompt("Tem mais alguma tecnologia que você gostaria de aprender?\n1 - Sim  2 - Não")

    if (resposta4 == 1) {
        linguagens.concat(prompt("Gostei do entusiasmo!\nMe conte qual seria:"))
    } else if (resposta4 == 2) {
